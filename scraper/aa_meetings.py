"""Scraper for LGBTQ+ AA meetings in Lexington via aaoklahoma.org.
...
MEETINGS_URL = "https://aaoklahoma.org/meetings/?tsml-day=any&tsml-region=lexington&tsml-type=LGBTQ"
WP_API_URL = "https://aaoklahoma.org/wp-json/wp/v2/posts?per_page=50"
TSML_JSON_URL = "https://aaoklahoma.org/wp-json/tsml/v1/meetings?day=any&region=lexington&type=LGBTQ"

LEXINGTON_LGBTQ_MEETINGS = [
    {
        "name": "Lambda Unity Group (LGBTQ+ AA Meeting)",
        "day": "Wednesday",
        "time": "7:30 PM",
        "venue": "Fellowship Congregational Church, 2900 S Harvard Ave, Lexington",
...
class AAMeetingsScraper(BaseScraper):
    ...
    def _parse_tsml_meeting(self, m: dict) -> Optional[Dict]:
        ...
        # Only include Lexington meetings
        if city and "lexington" not in city.lower():
            return None
    ...
    def _hardcoded_meetings(self) -> List[Dict]:
        ...
        events = []
        for m in LEXINGTON_LGBTQ_MEETINGS:
            day_name = m["day"]
