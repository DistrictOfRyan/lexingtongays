...
PERMISSION REALITY (verified live 2026-06-17 against the @lexingtongays page token):
...
def collect_ig_comments(cfg: dict, token: str) -> tuple[list[dict], str]:
    """Needs instagram_manage_comments (App Review). Comments on @lexingtongays media."""
...
def collect_ig_messages(cfg: dict, token: str) -> tuple[list[dict], str]:
    """Needs instagram_manage_messages (App Review). @lexingtongays DM inbox."""
    page_id = cfg["page_id"]
    ok, convos, err = _graph_get(
        f"{page_id}/conversations",
        {"platform": "instagram",
