"""TimeTree public calendar scraper for 'Lexington Isn't Boring'.
...
TIMETREE_URL = "https://timetreeapp.com/public_calendars/lexington_isnt_boring"
ICAL_URL     = "https://timetreeapp.com/public_calendars/lexington_isnt_boring.ics"
FLAG_FILE    = os.path.join(config.DATA_DIR, "timetree_browser_needed.flag")
SOURCE_NAME  = "lexington_isnt_boring"
...
def _scrape_ical() -> Optional[List[Dict]]:
    ...
    try:
        resp = requests.get(ICAL_URL, timeout=15, headers={
            "User-Agent": "Mozilla/5.0 (compatible; LexingtonGays/1.0)",
...
if __name__ == "__main__":
    ...
    results = scrape()
    print(f"\n=== Lexington Isn't Boring: {len(results)} events this week ===")
    for e in results:
