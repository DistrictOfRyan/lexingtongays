...
Reuses the proven @lexingtongays session loader in posting/ig_api.py. If the session is
...
Auto-reply guardrails (William chose auto-send-with-guardrails, 2026-06-17):
  ...
  - anonymous community voice only (no operator self-ID — the @lexingtongays hard rule)
...
DEFAULT_REPLY = (
    "Added to the LexingtonGays calendar, thank you for the tip! 🏳️‍🌈 "
    "You can see every Lexington LGBTQ+ event any time at lexingtongays.com. "
...
def get_client():
    """Logged-in @lexingtongays instagrapi Client, or None if the session is invalid."""
    try:
...
def _is_anonymous(text: str) -> bool:
    """Guard: the reply must not leak the operator (the @lexingtongays hard rule)."""
    low = text.lower()
...
def _selftest():
    ...
    class FakeUser:
        username = "lexingtonnbar"

    class FakeThread:
        id = "thread123"
        users = [FakeUser()]
        messages = [
            FakeMsg("m1", "Drag night at The Lexingtonn this Friday June 19, doors 9pm!"),
    ...
    tip = {"id": "tip001", "submitted_by": "lexingtonnbar", "thread_id": "thread123"}
