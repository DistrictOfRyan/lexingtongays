...
- Black Queer Lexington (Squarespace)
- All Souls Unitarian (JS-rendered calendar at /events-calendar)
- Eventbrite (JS-rendered event cards)
- Visit Lexington (JS-rendered calendar, LGBTQ keyword filter applied after load)
...
class BlackQueerLexingtonScraper(PlaywrightBaseScraper):
    """Black Queer Lexington -- Squarespace events page (JS-rendered)."""
...
class EventbriteJSScraper(PlaywrightBaseScraper):
    """Eventbrite LGBTQ Lexington search -- JS-rendered event cards."""
    ...
    def scrape(self) -> List[Dict]:
        ...
        for stub in event_stubs[:20]:
            ...
            found = self._extract_json_ld_from_soup(soup, "Lexington, KY", self.PRIORITY)
            if found:
                for e in found:
    ...
    def _parse_eventbrite_card(self, card) -> Optional[Dict]:
        ...
        venue = venue_el.get_text(strip=True) if venue_el else "Lexington, KY"
...
class VisitLexingtonScraper(PlaywrightBaseScraper):
    """Visit Lexington events calendar -- filter for LGBTQ-relevant events."""

    source_name = "visit_lexington"
    BASE_URL = "https://www.visitlexington.com"
    EVENTS_URL = "https://www.visitlexington.com/events/"
    PRIORITY = 3
    ...
    def scrape(self) -> List[Dict]:
        ...
        events = self._extract_json_ld_from_soup(soup, "Visit Lexington", self.PRIORITY)
        if not events:
    ...
    def _extract_generic_events(self, soup) -> List[Dict]:
        """Generic event card parser for Visit Lexington's JS-rendered layout."""
        containers = (
...
class PhilbrookMuseumScraper(PlaywrightBaseScraper):
    ...
    DEFAULT_VENUE = "Philbrook Museum of Art, 2727 S Rockford Rd, Lexington"
    PRIORITY = 2
