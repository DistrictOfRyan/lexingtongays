...
def run_pre():
    ...
    step("Generate carousel (rule-based)", [PY, "main.py", "generate-all"],
         timeout=900, env=_env(LEXINGTONGAYS_RULE_ENRICH="1"))
    ...
    # re-render with LEXINGTONGAYS_SKIP_ENRICH so the copy survives. Non-fatal: if the
...
def run_post(dry=False):
    ...
    step("Update website (gen_website_html)", [PY, "tools/gen_website_html.py"],
         timeout=600, env=_env(LEXINGTONGAYS_SKIP_ENRICH="1"))
    if dry:
    ...
    if not (getattr(config, "LEXINGTONGAYS_PAGE_ACCESS_TOKEN", "") or "").strip():
        print("  [X] Meta page token missing — cannot post FB/IG. Set "
              "LEXINGTONGAYS_PAGE_ACCESS_TOKEN and re-run --post.", flush=True)
        sys.exit(1)
    # 5. Post to FB page + Instagram (graphics).
    step("Post to FB + Instagram", [PY, "tools/post_weekly.py"],
         timeout=900, env=_env(LEXINGTONGAYS_SKIP_ENRICH="1"))
    ...
    #     draft for William to review and send. (Deepen-Lexington rung, 2026-06-15.)
