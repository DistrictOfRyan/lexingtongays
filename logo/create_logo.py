"""Generate Lexington Gays branding assets in Art Deco / Mid-Century Modern style.
...
def create_cover_image(width=1500, height=500):
    ...
    # "LEXINGTON GAYS" centered
    lexington_font = _font("impact", 120)
    gays_font = _font("impact", 120)

    # Measure for centering as one unit
    t_bbox = draw.textbbox((0, 0), "LEXINGTON ", font=lexington_font)
    g_bbox = draw.textbbox((0, 0), "GAYS", font=gays_font)
    total_w = (t_bbox[2] - t_bbox[0]) + (g_bbox[2] - g_bbox[0])
    start_x = (width - total_w) // 2
    y = 130

    draw.text((start_x, y), "LEXINGTON ", font=lexington_font, fill=DARK_GOLD)
    ...
    _centered(draw, "YOUR WEEKLY LGBTQ+ EVENT GUIDE FOR LEXINGTON", y + title_h + 60,
    ...
    _centered(draw, "Nothing to do in Lexington? Sounds like a straight person problem.",
...
if __name__ == "__main__":
    ...
    profile_path = os.path.join(output_dir, "lexingtongays_profile.png")
    profile.save(profile_path, "PNG", optimize=True)
    # Small version
    small = profile.resize((320, 320), Image.LANCZOS)
    small.save(os.path.join(output_dir, "lexingtongays_profile_320.png"), "PNG")
    print(f"Profile: {profile_path}")

    # Cover image
    cover = create_cover_image(1500, 500)
    cover_path = os.path.join(output_dir, "lexingtongays_cover.png")
