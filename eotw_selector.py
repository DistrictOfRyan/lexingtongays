...
_SKIP_VENUES = {
    "discovery lab",          # Lexington children's science museum (Big Build Week etc.)
...
# keyword (e.g. "Fun Friday Drags Night @ Lexington Raceway Park", "Motorama at the
...
def _is_lgbtq_strict(e: Dict) -> bool:
    ...
    # portable; fall back to the Lexington list if config is unavailable.
...
# Lexington gay bars / queer venues — by address and name. An event here counts as
...
_GAY_VENUE_SIGNATURES = (
    ...
    # Yellow Brick Road — Lexington's only lesbian bar, inclusive/everyone-welcome.
...
# ---------------------------------------------------------------------------
# The site has cross-promo deals (PFLAG Lexington, KYns for Equality / the
...
def _selftest() -> int:
    ...
    events = [
        ...
        {"name": "PFLAG Lexington: Big Gala", "source": "manual", "date": fri, "url": "x", "description": "gala"},
    ...
    if not any("PFLAG Lexington: Big Gala" in n for n in names):
