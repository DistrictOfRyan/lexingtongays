"""Discover new LGBTQ+ event sources in Lexington.
...
def discover_new_sources():
    """Search for new LGBTQ+ event sources in Lexington.
...
def evaluate_source(url):
    ...
    try:
        ...
        # Check for Lexington relevance (max 15 points)
        lexington_count = text.count("lexington")
        if lexington_count > 0:
            lexington_score = min(15, lexington_count * 5)
            score += lexington_score
            details.append(f"lexington_mentions={lexington_count}")
...
if __name__ == "__main__":
    print("=== Lexington Gays Source Discovery ===")
