...
def fetch_page(url: str) -> str:
    ...
    try:
        import requests
        headers = {"User-Agent": "Mozilla/5.0 (compatible; LexingtonGaysBot/1.0)"}
...
def _selftest():
    ...
    assert looks_like_calendar("https://example.com/events/")
    assert not looks_like_calendar("https://example.com/about")

    html = """<html><head><title>Drag Night</title>
    <script type="application/ld+json">
    {"@type":"Event","name":"Drag Night at The Bar Complex",
     "startDate":"2026-06-19T21:00:00-05:00",
     "location":{"@type":"Place","name":"The Bar Complex"},
     "description":"A wild Friday drag show."}
    </script></head><body></body></html>"""
    p = parse_event_from_html(html)
    assert p["name"] == "Drag Night at The Bar Complex", p
    assert p["date"] == "2026-06-19", p
    assert p["time"] == "9:00 PM", p
    assert p["venue"] == "The Bar Complex", p

    # enrich fills only empty fields; submitter's typed name wins
    d = tempfile.mkdtemp()
    cand_p = os.path.join(d, "cands.json")
    entry = {"id": "tip001", "channel": "ig", "name": "Bar Complex Drag (typed)",
             "date": "", "time": "", "venue": "", "description": "",
             "url": "https://example.com/events/drag", "raw_text": ""}
    entry = enrich_tip(entry, fetcher=lambda u: html, candidates_path=cand_p)
    assert entry["name"] == "Bar Complex Drag (typed)", entry   # typed name preserved
    assert entry["date"] == "2026-06-19", entry                 # date filled from page
    assert entry["venue"] == "The Bar Complex", entry
    ...
    entry2 = {"id": "tip002", "channel": "ig", "name": "x", "url": "https://example.com/events/drag2",
