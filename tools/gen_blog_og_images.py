"""Generate 1200x630 og:image banners for each Lexington Gays blog article.
...
Design language matches image_maker.py:
  ...
  - Site name strip: "lexingtongays.com" in Neon Pink, bottom center
  - Bottom accent bar: Neon Pink 4px
  - Logo: placed top-right if lexingtongays_profile.png exists in logo/
...
ARTICLES = [
    {
        "slug":     "gay-lexington-travel-guide",
        "headline": "Gay Lexington Travel Guide: Is Lexington LGBTQ+ Friendly?",
        "tag":      "TRAVEL GUIDE",
    },
    {
        "slug":     "new-to-lexington-queer-starter-pack",
        "headline": "New to Lexington? Here's Your Queer Starter Pack",
        "tag":      "RELOCATION GUIDE",
    },
    {
        "slug":     "how-we-find-every-queer-event",
        "headline": "How We Find Every Queer Event in Lexington",
        "tag":      "BEHIND THE SITE",
    },
    {
        "slug":     "date-night-queer-lexington",
        "headline": "Date Night Ideas for Queer Couples in Lexington",
        "tag":      "DATE NIGHT",
    },
    {
        "slug":     "lgbtq-sports-lexington",
        "headline": "LGBTQ+ Sports in Lexington: Bowling, Kickball, Softball, and More",
...
def _load_logo() -> Image.Image | None:
    """Try to load lexingtongays_profile.png from the logo directory.
    Returns a resized (80x80) RGBA image, or None if not found."""
    candidates = [
        os.path.join(LOGO_DIR, "lexingtongays_profile.png"),
        os.path.join(LOGO_DIR, "profile.png"),
        os.path.join(LOGO_DIR, "lexingtongays_profile_320.png"),
    ]
...
def make_og_banner(headline: str, tag: str,
    ...
    Layout (top to bottom):
      ...
      ~120px bottom zone: "lexingtongays.com" gold/pink, subtle site line
    ...
    # ── Bottom strip: "lexingtongays.com" ─────────────────────────────────────
    f_site      = _font("poiret", 38)
    f_site_sub  = _font("segoe", 18)

    site_y = H - 98
    _draw_centered(draw, "lexingtongays.com", site_y, f_site, NEON_PINK)
    _draw_centered(draw, "Your source for queer life in Lexington, KY",
...
def main():
    ...
    logo = _load_logo()
    if logo:
        print("Logo loaded: using lexingtongays_profile.png")
    else:
