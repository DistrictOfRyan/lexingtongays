...
def build_email(week_key):
    ...
    subject = f"[LEXINGTON GAYS] Event list for approval - {date_range_label}"
    ...
    if events is None:
        body = (
            ...
            f"  {events_file}\n\n"
            f"Run 'python main.py scrape' from the lexingtongays project folder, then "
            f"re-run generate_approval_email.py to get your approval draft.\n\n"
            f"-- Lexington Gays Auto"
        )
        return subject, body
