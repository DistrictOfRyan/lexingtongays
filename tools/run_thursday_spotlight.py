...
spotlighted recently, write a short blog article, deploy to lexingtongays.com/blog/,
...
GAY_BAR_VENUES = {
    ...
    "majestic lexington": "Club Majestic",
    "lexington eagle": "Lexington Eagle",
...
def _article_html(title: str, slug: str, hero_p: str, sections: list[tuple[str, str]]) -> str:
    ...
    canonical = f"https://www.lexingtongays.com/blog/{slug}.html"
    hero_p = _sed(hero_p)
...
<title>{title_e} - Lexington Gays Blog</title>
<meta content="{title_e}: this week's queer Lexington spotlight on lexingtongays.com." name="description"/>
<meta content="index, follow" name="robots"/>
<meta content="{title_e}" property="og:title"/>
<meta content="This week's queer Lexington spotlight." property="og:description"/>
...
{{
  ...
  "datePublished": "{today}",
  "author": {{"@type": "Organization", "name": "Lexington Gays"}},
  "publisher": {{"@type": "Organization", "name": "Lexington Gays"}},
...
{sections_html}
<p><a href="https://www.lexingtongays.com/">See the rest of this week's queer Lexington events.</a></p>
</div>
</main>
</body>
</html>
...
def _compose_article(subject_key: str, featured: dict, related: list[dict]) -> tuple[str, str, list]:
    ...
    if is_venue:
        ...
        hero = (
            f"This week's queer Lexington spotlight is {canonical_venue}. "
            "It is one of the gay bars that anchors the Lexington scene, "
...
def main() -> int:
    ...
    hero = sections.pop(0)[1] if sections and sections[0][0].lower().startswith("hero") else (
        f"This week's queer Lexington spotlight: {title.removeprefix('Spotlight: ')}."
    )
    ...
    img = make_engagement_slide(
        ...
        post_type="spotlight",
        subhead="This week's queer Lexington spotlight",
    )
    ...
    public_blog_url = f"https://www.lexingtongays.com/blog/{slug}.html"
    ...
    caption = _sed(
        ...
        "#LexingtonGays #QueerLexington #LGBTQLexington"
    )
    ...
    if args.dry_run:
        ...
        subprocess.run(
            ["git", "commit", "-m", f"lexingtongays-thursday-spotlight: {slug}"],
            cwd=ROOT,
            check=True,
        )
        ...
        subprocess.run(
            ["git", "commit", "-m", f"lexingtongays-thursday-spotlight: log {slug}"],
            cwd=ROOT,
            check=True,
        )
        subprocess.run(["git", "push", "origin", "HEAD"], cwd=ROOT, check=True)
    ...
    log_engagement_event(
        week_key,
        {
            "task": "lexingtongays-thursday-spotlight",
            "fired_at": datetime.now().isoformat(timespec="seconds"),
            "week": week_key,
            "subject_key": subject_key,
            "slug": slug,
