...
def main() -> int:
    ...
    if not new_events:
        log_engagement_event(
            week_key,
            {
                "task": "lexingtongays-wednesday-lastminute",
    ...
    caption_lines += ["", "Full week at lexingtongays.com.", "#LexingtonGays #QueerLexington #LGBTQLexington"]
    ...
    if args.dry_run:
        ...
        subprocess.run(
            ["git", "commit", "-m", f"lexingtongays-wednesday-lastminute: {week_key}"],
            cwd=ROOT,
            check=True,
        )
