...
SELF_PROMO = "Find every Lexington LGBTQ+ event any time at lexingtongays.com 🏳️‍🌈"
...
def _add_promo_footer(img):
    """Draw a lexingtongays.com self-promo band across the bottom of a spotlight slide.
    ...
    try:
        ...
        text = "FIND EVERY LEXINGTON GAY EVENT  →  LEXINGTONGAYS.COM"
...
def _notify(summary: dict, queued: list):
    ...
    try:
        lines = [
            f"\n## [{datetime.now():%Y-%m-%d %H:%M}] LexingtonGays: {len(queued)} new event tip(s) auto-ingested (review-ready)",
        ]
        for e in queued:
            when = e.get("date") or "????-??-??"
...
def _selftest():
    ...
    dm_messages = [{
        ...
        "text": "Drag Night at The Tulsan Fri June 19 2026 9pm — flyers attached, "
                "more at https://www.lexpridecenter.org/pride-community-event-calendar",
