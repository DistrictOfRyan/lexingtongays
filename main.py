"""
Lexington Gays - Main Orchestrator
...
def cmd_scrape():
    ...
    # Pull emailed events (events@lexingtongays.com) into manual_events BEFORE scraping,
...
def _dedup_day(ev_list):
    ...
    def _fuzzy_same(a, b):
        ...
        Lexington)' vs '... in Lexington Arts District', WITHOUT merging distinct
    ...
    for ev in ev_list:
        ...
        # Lexington Artist Fellowship First Fridays / Flagpole Go-Go events are
        ...
        _taf_first_friday = (
            ...
            and ('first friday' in name_norm or 'flagpole' in name_norm
                 ...
                 or 'lexington artist fellowship' in venue_norm
        ...
        if idx is None:
            ...
            if new_venue and not old_venue:
                ...
                # The kept venue is all generic/geographic words ("Lexington, KY") --
...
def cmd_generate(post_type="weekday"):
    ...
    # LEXINGTONGAYS_SKIP_ENRICH=1 renders slides straight from the copy already in
    ...
    if os.environ.get("LEXINGTONGAYS_SKIP_ENRICH", "").strip().lower() in ("1", "true", "yes"):
        print("\nLEXINGTONGAYS_SKIP_ENRICH set — rendering from existing copy (no re-enrichment)")
    else:
    ...
    _ALWAYS_DEPRIORITIZE = {
        ...
        "aa meetings",
        "book club - lexington",  # org-specific book clubs (Lexington SWE, etc.)
    ...
    def _slide_priority(e):
        ...
        # so events like "Zoolightful at Lexington Zoo" and "Open Meditation"
...
def cmd_full_run():
    ...
    print("*" * 60)
    print("  LEXINGTON GAYS - FULL WEEKLY RUN")
...
def cmd_test():
    ...
    print("*" * 60)
    print("  LEXINGTON GAYS - TEST RUN (no posting)")
    print("*" * 60)
...
def _fallback_caption(events, post_type, date_range):
    ...
    lines = [f"Here's what's happening {period} in Lexington! ({date_range})\n"]
