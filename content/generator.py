...
def _pick_hashtags(count: int = 18) -> list[str]:
    ...
    must_have = ["#LexingtonGays", "#HomoHotelHappyHour", "#LexingtonLGBTQ"]
...
You are writing AS a member of the Lexington queer community who genuinely cares \
...
def enrich_event_descriptions(events: list[dict]) -> list[dict]:
    ...
    for batch_start in range(0, len(needs_enrichment), BATCH):
        ...
        prompt = (
            ...
            "Warm, Lexington queer community voice. Use any hint provided to be more specific.\n\n"
            "Events:\n" + "\n".join(event_lines) +
        ...
        try:
            message = client.messages.create(
                ...
                max_tokens=1200,
                system=(
                    "You write event descriptions for LexingtonGays.com, an LGBTQ+ community events guide. "
...
def _rule_based_enrich(event: dict) -> str:
    ...
    if any(b in _venue_lower for b in _true_bars) or any(b in name for b in _true_bars) or src in ("bars", "nightlife"):
        return (f"Get there{at_time}, order your drink, and start a conversation with whoever's at the bar next to you. "
                "Lexington's queer nightlife runs on community, and the community only stays strong when you show up.")
    ...
    if any(k in name for k in ["market", "art market", "art show", "art fair", "gallery"]):
        return (f"Bring cash. Budget a little more than you think you'll spend. Talk to the artists. Ask them about their work. "
                "This is where Lexington's creative community lives, and it's better than anything you'd find scrolling at home.")
    ...
    if any(k in name for k in ["concert", "live music", "music night", "performance"]):
        return (f"Arrive before it starts. Find a spot. Put the phone away. "
                "Live music in Lexington is genuinely underrated and you are going to feel something if you actually let yourself be present for it.")
    ...
    if (any(k in name for k in ["unitarian", "church", "spiritual", "meditation"])
        ...
        return (f"One of the most affirming spaces in Lexington for queer people of faith and queer skeptics alike. "
    ...
    if src in _sig_org_keys or "equality center" in (venue or "").lower():
        return (f"This space{at_venue} is the heartbeat of Lexington's queer community. "
...
def _fallback_caption(
    ...
    hooks = [
        ...
        f"lexington queer events for {date_range or 'this week'}",
