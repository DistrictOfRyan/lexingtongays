...
def _pick_hashtags(count: int = 10) -> list[str]:
    ...
    must_have = getattr(config, "MUST_HAVE_HASHTAGS",
                        ["#LexingtonGays", "#LexingtonLGBTQ", "#HomoHotelHappyHour"])
...
_SYSTEM_PROMPT = """\
You are the social media voice of Lexington Gays, a local Instagram account that \
posts LGBTQ+ event roundups for Lexington, KY. You are NOT an AI assistant. \
You are writing AS a member of the Lexington LGBTQIA+ community who genuinely cares \
...
- Occasionally drop a Lexington-specific reference (Brookside, the Gathering Place, Cherry Street, 11th & Lewis, Philbrook, etc)
...
def generate_post_caption(
    ...
    user_prompt = f"""\
Write an Instagram caption for the Lexington Gays account.
...
# A `claude -p` default-auth 401 once silently killed EVERY lexingtongays CLI LLM
...
def _call_claude_cli(user_prompt: str, system_prompt: str = "", model: str = "sonnet",
    ...
    if system_prompt:
        ...
        merged = user_prompt
    # Run from a neutral cwd. From inside the lexingtongays/ project, `claude -p`
...
_YBR_INCLUSIVE_TAG = (
    "Yellow Brick Road is Lexington's only lesbian bar and one of the last in the US, "
...
VOICE_SYS_PROMPT = (
    "You write event descriptions for LexingtonGays.com, an LGBTQ+ community events guide in "
    "Lexington. Voice: RuPaul meets Alicia Edwards (Abbott Elementary) with a warm Dolly Parton "
...
def voice_enrich(events: list[dict], budget_s: int = 240, batch: int = 6) -> dict:
    ...
    with LEXINGTONGAYS_SKIP_ENRICH keeps the copy. Returns a stats dict.
...
def _enrich_event_descriptions_impl(events: list[dict]) -> list[dict]:
    ...
    #   1) LEXINGTONGAYS_RULE_ENRICH=1 forces the fast, deterministic rule-based path
    ...
    #   2) LEXINGTONGAYS_ENRICH_BUDGET_S caps total CLI wall-clock; once spent, every
    ...
    if os.environ.get("LEXINGTONGAYS_RULE_ENRICH", "").strip().lower() in ("1", "true", "yes"):
        print("[generator] LEXINGTONGAYS_RULE_ENRICH set — fast rule-based enrichment (no nested claude CLI)")
        return _rule_based_enrich_all(events)
    _budget_s = int(os.environ.get("LEXINGTONGAYS_ENRICH_BUDGET_S", "240"))
...
_VOICE_BANK = {
    "drag": [
        ...
        "Darling, this is the closest thing Lexington has to church. Front row, fresh dollar "
        ...
        "This is where Lexington keeps its glamour, honey. Big wigs, bigger personalities, and a "
    ...
    "bar": [
        ...
        "beside you like y'all go way back. Lexington's queer nightlife runs on exactly that "
        ...
        "This is where Lexington's queer crowd unwinds and it shows, baby. Roll in before the rush "
    ...
    "music": [
        ...
        "once, honey. Lexington's live music is criminally underrated. Early gets you close; close "
        "gets you a story.",
...
def _rule_based_enrich(event: dict) -> str:
    ...
    _scraper_artifacts = [
        "lexington events lists", "ticket options may be available",
        "verified providers", "events.lexington.okstate.edu",
        "did you know that **", "this event is sold out this is not an official",
    ]
    # Strip OKEQ-style metadata prefix. Two patterns to handle:
    #   "May 28, 2026 | 6:00 pm - 8:00 pm Dennis R. Neill Equality Center, 621 E 4th St, Lexington, KY 74120, USA <real desc>"
    ...
    if any(k in name for k in ["cabaret", "chorus", "chorale"]) or any(kw in name for kw in _anchor_kws):
        return (f"Dress up. Get there early. Sit close. "
                "This is live performance from a real ensemble that pours everything into it, and the energy in that room is unlike anything else in Lexington.")
    ...
    if (any(k in name for k in ["unitarian", "church", "spiritual", "meditation"])
        ...
        return (f"One of the most affirming spaces in Lexington for LGBTQIA+ people of faith and skeptics alike. "
    ...
    if "$5 wednesday" in name and "mother road" in (venue.lower() + name):
        return ("Five bucks for a midweek reset at Mother Road. Vendors, food trucks, live entertainment, "
                "and the kind of crowd that makes you remember Lexington actually rules. Best five dollars you'll spend this week.")
    if "musical bikes" in name:
        return ("Lexington's most casual cycling crew rolls through Mother Road for Musical Bikes Monday. "
    ...
    if "magic city books" in venue.lower():
        return ("Magic City Books is Lexington's queer-friendly indie bookstore and they pour everything into their author events. "
    ...
    if "shambhala" in venue.lower() or ("meditation" in name and "shambhala" in (venue.lower() + name)):
        return ("Shambhala is genuinely one of the most welcoming meditation spaces in Lexington, full stop. "
    ...
    if "neff brewing" in venue.lower() or ("dinner" in name and "courses" in name):
        return ("A six-course tasting paired with NEFF's craft brews. Ticketed, intimate, and unlike anything else "
                "happening in Lexington that night. If you've never done a curated tasting menu, this is the one.")
    ...
    if "guthrie green" in venue.lower():
        return ("Guthrie Green is Lexington's downtown front yard. Show up, find a spot, talk to whoever sits near you. "
                "Public space done right.")

    # Zoolightful / zoo events
    if "zoolightful" in name or ("zoo" in venue.lower() and "lantern" in name):
        return ("Lexington Zoo after dark, lit up with hundreds of illuminated animal lanterns. Bring a date, "
    ...
    if "morecolor" in name:
        return ("Allie Jensen Gallery transforms into one of Lexington's best art receptions twice a year. "
    ...
    if "broadway clubhouse" in name or "broadway club house" in name:
        return ("Lexington's monthly queer sing-along in the Lynn Riggs Theater at the Equality Center. "
    ...
    if src in _sig_org_keys or "equality center" in (venue or "").lower():
        return (f"This space{at_venue} is the heartbeat of Lexington's LGBTQIA+ community. "
...
def _rule_based_website_description(event: dict, short: str) -> str:
    ...
    closer = _pick((event.get("name") or "") + "L", [
        "Put on the look, bring a friend or bring your nerve, and let Lexington show you a good time.",
...
def _fallback_caption(
    ...
    hooks = [
        ...
        f"lexington lgbtqia+ events for {date_range or 'this week'}",
